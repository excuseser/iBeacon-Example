...
@interface KRBeaconFinder ()<CLLocationManagerDelegate, UIAlertViewDelegate>
...
-(void)bleScanWithUUID:(NSString *)UUID{
    [_beaconCentral scanWithUUID:UUID];
}
...
-(void)locationManager:(CLLocationManager *)manager didChangeAuthorizationStatus:(CLAuthorizationStatus)status
{
    ...
    switch (authorizationStatus) {
        ...
        case kCLAuthorizationStatusAuthorizedWhenInUse:
            NSLog(@"Couldn't turn on monitoring: Required Location Access(Always) missing. kCLAuthorizationStatusAuthorizedWhenInUse");
            return;
...
- (void)checkLocationAccessForMonitoring {
    if ([self.locationManager respondsToSelector:@selector(requestAlwaysAuthorization)]) {
        ...
        if (authorizationStatus == kCLAuthorizationStatusDenied ||
            ...
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"位置访问错误"
                                                            message:@"请设置允许位置访问"
                                                           delegate:self
    ...
    if ([[UIApplication sharedApplication] backgroundRefreshStatus] == UIBackgroundRefreshStatusAvailable) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"后台数据获取错误"
                                           message:@"请设置允许后台位置刷新"
                                          delegate:self
                                 cancelButtonTitle:@"设置"
                                 otherButtonTitles:@"取消", nil];
...
#pragma mark - 通过rssi计算距离
- (float)calcDistByRSSI:(int)rssi{
    int iRssi = abs(rssi);
    float power = (iRssi-59)/(10*2.0); //59为1米的参考值
    return pow(10, power);
}
